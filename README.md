# 👋 Hi there! Welcome to my GitHub Profile

## 🚀 About Me
- 🎓 Computer Science student from Chennai
- 💻 Skills: Python 🐍 | Java ☕ | SwiftUI 🍏 | C 💡
- 📘 Currently Learning: C++ ➕➕ and Machine Learning 🤖
- ⚽ I love to play football
- 💡 Hobbies: Exploring tech, gadgets, and new trends

## 💼 What I'm Currently Working On
- Building Java projects for coursework and self-learning
- Exploring open-source contributions on GitHub
- Football content creation and social media

## 🌱 What I'm Currently Learning
- C++ fundamentals and intermediate concepts
- Machine Learning basics, models and applications

## 👯 Looking to Collaborate On
- Web and Android development projects
- Open Source software in education or health
- Football and anime-themed apps/platforms

## 🤔 Looking for Help With
- Advanced algorithms in Java and Python
- DevOps and cloud platforms
- Best practices in database design

## 📬 Contact Me

[![Mail](https://img.shields.io/badge/Mail-green?logo=gmail&logoColor=white)](mailto:nithispandiyan24110332@snuchennai.edu.in)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0072b1?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/nithispandiyan/)

## ⚡ Fun Fact
- Can solve a Rubik's Cube
- Likes to find correlations between football strategies and coding!

---

### 🛠️ Technologies & Tools
- Python | Java | C | SwiftUI | MySQL | Git | VS Code | Android Studio

### 📊 GitHub Stats
- Recently contributed to open source repos
- Building an education-focused Java project


### 🎯 Goals for 2025
- Master C++ and Machine Learning
- Successfully publish an open-source football dashboard

---

![Visitors](https://komarev.com/ghpvc/?username=nithis-057)
